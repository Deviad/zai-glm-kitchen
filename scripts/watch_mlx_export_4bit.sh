#!/usr/bin/env bash
# Background watcher: polls 4-bit MLX export status every 60s.
set -euo pipefail

KITCHEN="/Volumes/Data NVME/GLM-5.2-kitchen"
OUT_DIR="/Volumes/Data NVME/GLM-5.2-MLX/GLM-5.2-shortgpt-pruned-4bit-mlx"
WATCH_LOG="$KITCHEN/logs/mlx_export_4bit_watch.log"
echo $$ > "$KITCHEN/logs/mlx_watch_4bit.pid"

echo "Watch started at $(date)" >> "$WATCH_LOG"

while true; do
    PID=$(cat "$KITCHEN/logs/mlx_export_4bit.pid" 2>/dev/null || echo "?")
    if [ "$PID" = "?" ]; then
