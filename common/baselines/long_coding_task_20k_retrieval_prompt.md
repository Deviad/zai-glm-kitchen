...
You have read a long coding task above. This is a retrieval/retention test, not a coding request.

Answer only the following three lines, with no markdown code block and no extra explanation:

sentinel: <the exact IMPORTANT SENTINEL string from near the beginning>
function: <the exact function name requested in the main coding task>
